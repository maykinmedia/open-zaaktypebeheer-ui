...
const DashboardView = () => {
  ...
  const filteredResults = filter.attributeOnQuery(query, value?.results!, 'omschrijving');
  const zaaktypen: ZaaktypeT[] = !filteredResults ? new Array(4).fill({}) : filteredResults; // Used to show skeleton loading
  ...
  return (
    <Stack component={'article'} spacing={spacing} useFlexGap>
      ...
      {dataVisualLayout === 'datagrid' && <DataGrid data={zaaktypen} />}
